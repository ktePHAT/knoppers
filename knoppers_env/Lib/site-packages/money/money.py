...
class Money:
    ...
    def sub_units(self) -> int:
        ...
        sub_units_per_unit = CurrencyHelper.sub_unit_for_currency(
            self.currency)
    ...
    def __floordiv__(self, other: Union['Money', float]) -> Union['Money', float]:
        if isinstance(other, Money):
            ...
            if other == 0:
                raise ZeroDivisionError
            amount = self._round(
                self._amount // Decimal(other), self._currency)
            return self.__class__(str(amount), self._currency)
    ...
    def _round(amount: Decimal, currency: Currency) -> Decimal:
        ...
        decimal_precision = CurrencyHelper.decimal_precision_for_currency(
            currency)
